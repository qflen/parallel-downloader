...
| §14.4 | `If-Range` with ETag (strong validator) | Covered | Forwarded to every chunk request whenever the probe yielded an ETag; a server must reply 206 if still valid, 200 otherwise. We treat the 200 reply as a fatal `HttpError` so a mid-download file change cannot splice two file versions silently. | `IfRangeTest` |
| §14.4 | `If-Range` with `Last-Modified` (weak validator) | Covered | Same path; falls back when no ETag is present. | `IfRangeTest` |
...
| §15.3.7 | 206 Partial Content | Covered | Required status for a ranged GET. A 200 reply to a ranged GET is treated as a non-retryable protocol violation rather than spliced into the destination. | `JdkHttpRangeFetcherTest`, `IfRangeTest` |
...
| §13.1.5 | `If-Range` | Covered | The downloader's safety anchor against mid-download file change. See §14.4 row above. | `IfRangeTest` |
...
| 200 | success | ranged GET reply | reject as fatal protocol violation |
